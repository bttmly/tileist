...
$.fn.tileist = function(options) {
  ...
  */
	...
	;
  var Color, TwoDimensionalArray, cell, color, column, defaults, distanceOf, distances, focus, gridSize, htmlStr, normalize, randomIntFromInterval, settings,
    _this = this;
  ...
  */
  ...
  normalize = function(numbers, min, max) {
    var maxVal, minVal, num, _i, _j, _len, _len1, _results;
    if (min == null) {
      min = 0;
    }
    if (max == null) {
    ...
    for (_j = 0, _len1 = numbers.length; _j < _len1; _j++) {
      num = numbers[_j];
      _results.push((num - minVal) / (max - min));
    }
    return _results;
  };
  ...
  grid.cells.each(function(i) {
    ...
    colorize = function($el) {
      ...
      method = num < 50 ? "lightenByRatio" : "darkenByRatio";
      $el.css({
